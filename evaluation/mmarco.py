...
class EvaluationMMARCO:
    ...
    def __init__(
        ...
        for qrel in relevances:
            # 1 relevant, 2 highly relevant, 3 perfectly relevant
            if qrel.relevance in [1, 2, 3]:
        ...
        for qrel in relevances:
            if qrel.relevance in [1, 2]:
                if qrel.query_id not in self.query_doc_score_relevances:
                    self.query_doc_score_relevances[qrel.query_id] = {
    ...
    def evaluate(self, metrics=["hit"], scores_at=[1, 5, 10, 20, 100]):
            ...
            if query_id in self.relevance_dictionary:
                ...
                for r in scores_at:
                    ...
                    if r == scores_at[-1]:
                        ...
                        # get perfect ranking of the relevant documents for
                        # the given query
                        ...
                        for rank_ in range(1, len(pred_documents)):
                            ...
                            if len(perfect_sort) - 1 >= rank_:
                                PERFECT_DCG.append(perfect_sort[
        ...
        if self.ranking_method.method_name == "random":
            ...
            result_path = (
                "Results/mmarco("+self.ranking_method.method_name+").pkl")
